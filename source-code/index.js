import {
  ...
  doApprove, doSwap, calculateEstimateGasExchangeToken, calculateEstimateGasApprove
...
$('#confirm-swap-button-mix-modal').click(function () {
  ...
  doSwap(srcToken.address, destToken.address, account, amount * Math.pow(10, 18).toString(), 0, web3, fn);
})
...
$('#confirm-swap-button').click(function () {
  ...
  if (srcToken.address !== nativeToken.address && destToken.address !== nativeToken.address) {
    ...
    doSwap(srcToken.address, destToken.address, account, amount * Math.pow(10, 18).toString(), amount * Math.pow(10, 18).toString(), web3, fn);
  }
});
